...
describe('calendar', () => {
  describe('buildSaveTheDateCalendar', () => {
    it('spans one day before the first event through one day after the last', () => {
      ...
      // First event is the 11th → start on the 10th.
      expect(calendar?.start.toISOString()).toBe('2026-09-10T00:00:00.000Z')
      // Last event is the 12th → inclusive last day is the 13th → exclusive end is the 14th.
      expect(calendar?.endExclusive.toISOString()).toBe('2026-09-14T00:00:00.000Z')
    })
  ...
  describe('buildSaveTheDateCalendarLinks', () => {
    ...
    it('builds an all-day Google Calendar link', () => {
      ...
      expect(links?.googleUrl).toContain('dates=20260910%2F20260914')
    ...
    it('builds an all-day Outlook link', () => {
      ...
      expect(links?.outlookUrl).toContain('startdt=2026-09-10')
      expect(links?.outlookUrl).toContain('enddt=2026-09-14')
    })

    it('builds an all-day .ics file', () => {
      expect(links?.fileName).toBe('save-the-date.ics')
      expect(links?.ics).toContain('BEGIN:VEVENT')
      expect(links?.ics).toContain('DTSTART;VALUE=DATE:20260910')
      expect(links?.ics).toContain('DTEND;VALUE=DATE:20260914')
