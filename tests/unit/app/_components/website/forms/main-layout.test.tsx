...
jest.mock('~/app/_components/hooks', () => ({
  useConfirmReloadPage: jest.fn(),
}))

jest.mock('~/app/_components/website/forms/multi-step-form', () => {
  return function MockMultistepRsvpForm({ children }: { children: React.ReactNode }) {
    return <>{children}</>
  }
})

jest.mock('~/app/_components/website/forms/steps/find-your-invitation', () => {
  return function MockFindYourInvitationForm() {
    return <div>Find invitation</div>
  }
})

jest.mock('~/app/_components/website/forms/steps/confirm-name', () => {
  return function MockConfirmNameForm() {
    return <div>Confirm name</div>
  }
})

jest.mock('~/app/_components/website/forms/steps/event-rsvp', () => {
  return function MockEventRsvpForm() {
    return <div>Event RSVP</div>
  }
})

jest.mock('~/app/_components/website/forms/steps/question-multiple-choice', () => {
  return function MockQuestionMultipleChoice() {
    return <div>Question multiple choice</div>
  }
})

jest.mock('~/app/_components/website/forms/steps/question-short-answer', () => {
  return function MockQuestionShortAnswer() {
    return <div>Question short answer</div>
  }
})

jest.mock('~/app/_components/website/forms/steps/send-rsvp', () => {
  return function MockSendRsvp() {
    return <div>Send RSVP</div>
  }
})

jest.mock('~/app/_components/website/rsvp-confirmation', () => {
