...
test.describe("Guest List Drawer - Viewing Details", () => {
  ...
  test("should open the guest detail drawer when clicking a household card", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /donkey the donkey/i })
    ).toBeVisible();
  });
  ...
  test("should display contact information for Donkey household", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /select donkey.*household/i })
  ...
  test("should display party members with names and age groups", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /select donkey.*household/i })
  ...
  test("should show notes in the drawer", async ({ page }) => {
    ...
    await expect(
      page.getByText(/Needs high-chair seating for dragonlets/i)
    ).toBeVisible();
  });
  ...
  test("should close the drawer when clicking the close button", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /select donkey.*household/i })
  ...
  test("should show different details for different households", async ({
    page,
  }) => {
    // Open Gingy household
    await page
      .getByRole("button", { name: /select gingy.*household/i })
      .click();

    await expect(
      page.getByRole("heading", { name: /gingy cookie/i })
    ).toBeVisible();
  ...
  test("should show household with multiple members (Three Bears)", async ({
    page,
  }) => {
...
test.describe("Guest List Drawer - Editing Contact & Address", () => {
  ...
  test("should pre-fill contact fields with existing data", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /select donkey.*household/i })
  ...
  test("should toggle notes editing mode", async ({ page }) => {
    ...
    await expect(notesTextarea).toHaveValue(
      "Needs high-chair seating for dragonlets."
    );
  });
});
...
test.describe("Guest List Drawer - Managing Members", () => {
  ...
  test("should open the manage members modal", async ({ page }) => {
    ...
    // Modal should appear
    await expect(
      page.getByRole("heading", { name: /manage household members/i })
    ).toBeVisible();
    await expect(
      page.getByText(
        /add, update, or remove people in this household before saving/i
      )
    ).toBeVisible();
  });

  test("should show existing members in the modal with correct names", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /select donkey.*household/i })
    ...
    await expect(page.getByLabel("First name (member 1)")).toHaveValue(
      "Donkey"
    );
    await expect(page.getByLabel("Last name (member 1)")).toHaveValue(
      "The Donkey"
    );
    await expect(page.getByLabel("First name (member 2)")).toHaveValue(
      "Dragon"
    );
    await expect(page.getByLabel("Last name (member 2)")).toHaveValue(
      "The Dragon"
    );
  });
  ...
  test("should show validation error when saving members with empty names", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /select donkey.*household/i })
    ...
    await expect(
      page.getByRole("button", { name: /save members/i })
    ).toBeDisabled();

    // Should show a validation message
    await expect(
      page.getByText(/must include a first and last name/i)
    ).toBeVisible();
  });
  ...
  test("should remove a member from the household", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: /remove dragon the dragon/i })
      .click();
  ...
  test("should not allow removing the only primary member", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("button", { name: /remove gingy cookie/i })
    ).toBeDisabled();
  });
  ...
  test("should set a different member as primary contact", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /select donkey.*household/i })
    ...
    await page
      .getByRole("button", { name: /set dragon the dragon as primary/i })
      .click();
...
test.describe("Guest List Drawer - Tags Management", () => {
  ...
  test("should open tags modal from the members modal", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /tags for donkey the donkey/i })
    ).toBeVisible();
    await expect(
      page.getByText(/select tags for this guest or create new ones/i)
    ).toBeVisible();
  });
  ...
  test("should show available tags from seed data", async ({ page }) => {
    ...
    await expect(
      page.getByLabel("Bridal Party", { exact: true })
    ).toBeVisible();
    await expect(page.getByLabel("VIP", { exact: true })).toBeVisible();
    await expect(
      page.getByLabel("Out of Town", { exact: true })
    ).toBeVisible();
  ...
  test("should cancel tag modal without changes", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /tags for donkey/i })
    ).not.toBeVisible();
    await expect(
      page.getByRole("heading", { name: /manage household members/i })
    ).toBeVisible();
...
test.describe("Guest List Drawer - Delete Party", () => {
  ...
  test("should show delete party button in the drawer", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /delete party/i })
    ).toBeVisible();
  });
  ...
  test("should show delete confirmation dialog", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /delete party\?/i })
    ).toBeVisible();
    await expect(
      page.getByText(
        /this will permanently delete this party and all associated guests/i
      )
    ).toBeVisible();
    await expect(page.getByRole("button", { name: "Cancel" })).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Delete Party" })
    ).toBeVisible();
  });

  test("should cancel deletion and keep the drawer open", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /select donkey.*household/i })
...
test.describe("Guest List Drawer - Discard Changes Dialog", () => {
  test("should show discard dialog when closing drawer with unsaved edits", async ({
    page,
  }) => {
    await page.goto("/guest-list");
    ...
    await expect(
      page.getByRole("button", { name: /discard and close/i })
    ).toBeVisible();
  });

  test("should return to editing when choosing keep editing", async ({
    page,
  }) => {
    await page.goto("/guest-list");
