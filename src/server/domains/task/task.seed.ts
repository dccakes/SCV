...
function freezeSeedEntry<const T extends CanonicalTaskSeed>(entry: T): Readonly<T> {
  return Object.freeze({ ...entry })
}

const TASK_SEED_ENTRIES: CanonicalTaskSeed[] = [
  freezeSeedEntry({
...
export const TASK_SEED: ReadonlyArray<CanonicalTaskSeed> = Object.freeze(
  TASK_SEED_ENTRIES.map((entry) => freezeSeedEntry(entry))
)

export function getCanonicalTaskSeed(): CanonicalTaskSeed[] {
  return TASK_SEED.map((task) => ({ ...task }))
}
