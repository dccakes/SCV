...
type QuestionOption = NonNullable<QuestionInput['options']>[number]

export const questionRouter = createTRPCRouter({
  upsert: protectedProcedure.input(questionInputSchema).mutation(async ({ input, ctx }) => {
    ...
    const upsertOptions = {
      upsert: input.options?.map((option: QuestionOption) => {
        return {
          where: {
            id: option.id ?? '-1',
          },
    ...
    const createOptions = {
      create: input?.options?.map((option: QuestionOption) => {
        return {
          text: option.text,
          description: option.description ?? '',
          responseCount: 0,
