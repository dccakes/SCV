...
type GuestListErrorProps = {
  error: Error & { digest?: string }
  reset: () => void
}

export default function GuestListError({ error, reset }: Readonly<GuestListErrorProps>) {
  return (
    <div className='mx-auto flex min-h-[50vh] w-full max-w-2xl flex-col items-center justify-center gap-4 px-4 text-center md:px-6'>
      <h2 className='font-semibold text-xl md:text-2xl'>Unable to load your guest list</h2>
      <p className='max-w-lg text-muted-foreground text-sm md:text-base'>
        We could not load your guests right now. Please try again.
      </p>
      {error.digest ? (
