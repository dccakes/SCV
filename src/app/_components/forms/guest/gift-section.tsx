...
export default function GiftSection({ control, register, errors, events }: GiftSectionProps) {
  ...
  return (
    <>
      <h2 className='my-4 font-bold text-2xl'>Gifts</h2>
      {fields.map((field, index) => {
        ...
        return (
          <div key={field.id} className='mb-6'>
            <h3 className='mb-3 font-semibold text-lg'>{eventName}</h3>
            <div className='flex flex-col gap-3'>
              ...
              <div>
                ...
                {giftErrors?.description && (
                  <p className='mt-1 text-red-600 text-sm'>{giftErrors.description.message}</p>
                )}
              </div>
            </div>
          </div>
