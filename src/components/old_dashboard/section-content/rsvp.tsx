...
import type { Event, Question } from '~/app/utils/shared-types'
  ...
  return (
    <div className='border-t'>
      {events?.map((event) => {
        ...
        return (
          <div key={event.id} className='border-b px-10 pt-10 pb-20'>
      ...
      <div className='p-10'>
        <h3 className='pb-6 font-semibold text-2xl'>General Questions</h3>
        <div className='grid grid-cols-2 gap-x-7 gap-y-20'>
  ...
  return (
    <div>
      <div className='mb-4 h-full rounded-lg border p-5'>
        <h5 className='pb-6 font-semibold text-lg'>Will you be attending?</h5>
        <div className='flex items-center justify-between gap-7'>
          <div className='relative h-full w-40'>
            <Doughnut data={getChartData(event.guestResponses)} options={chartOptions} />
            <div className='absolute top-1/2 left-1/2 flex -translate-x-1/2 -translate-y-1/2 flex-col justify-center text-center font-bold leading-tight'>
      ...
      <Link href={`/guest-list?event=${event.id}`} className='text-primary text-sm hover:underline'>
        Manage Guest List
      </Link>
    </div>
  )
...
const TextQuestionCard = ({ question }: { question: Question }) => {
  ...
  return (
    <div className='mb-4 flex h-full min-h-60 flex-col rounded-md border p-5'>
      <h5 className='font-semibold text-lg'>{question.text}</h5>
...
const OptionQuestionCard = ({ question }: { question: Question }) => {
  ...
  return (
    <div className='mb-4 h-full rounded-md border p-5'>
      <h5 className='pb-6 font-semibold text-lg'>{question.text}</h5>
      <div className='flex items-center justify-between gap-7'>
        <div className='relative h-full w-40'>
          <Doughnut data={getChartData()} options={chartOptions} />
          <div className='absolute top-1/2 left-1/2 flex -translate-x-1/2 -translate-y-1/2 flex-col justify-center text-center font-bold leading-snug'>
